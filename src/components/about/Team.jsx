...
function LocationSection() {function LocationSection() {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-6 md:gap-8 mb-6 md:mb-10 border-b border-amber-50 pb-6 md:pb-8 text-left w-full">
        
        {/* Infos Cabinet */}
        <div className="flex items-center md:items-start w-full lg:w-auto">
          <div className="p-3 md:p-4 bg-amber-50 rounded-xl md:rounded-2xl mr-4 md:mr-5 shrink-0">
            <MapPin className="w-6 h-6 md:w-7 md:h-7 text-amber-800" />
          </div>
          <div>
            <h3 className="text-xl md:text-2xl font-bold text-[#3e2723] mb-1 md:mb-2 font-serif">Notre cabinet</h3>
        ...
        <div className="flex flex-col items-center lg:items-end gap-3 md:gap-4 w-full lg:w-auto">
          <p className="text-[10px] md:text-[11px] text-amber-800/70 font-medium italic max-w-[280px] text-center lg:text-right">
            Note : Pour tracer votre itinéraire, autorisez Google Maps à accéder à votre position actuelle.
          </p>
          ...
          <a 
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-[#3e2723] hover:bg-[#1a120b] text-white px-6 md:px-8 py-3 md:py-4 rounded-xl font-bold text-sm md:text-base transition-all shadow-xl active:scale-95 group"
          >
            Tracer l'itinéraire
      ...
      {/* CARTE RESPONSIVE: 
          ...
          - h-[600px] sur desktop
      */}
      <div className="relative w-full h-[350px] md:h-[600px] rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border-2 md:border-4 border-white z-0">
        <MapContainer 
          center={position} 
          zoom={17} 
          ...
          <Marker position={position}>
            <Popup>
              <div className="text-center p-1">
                <strong className="text-[#3e2723] text-xs md:text-sm block">Cabinet Maître TCHOUHO</strong>
                <span className="text-[10px] text-amber-800 italic">Rue des Généraux, Yaoundé</span>
              </div>
            </Popup>
          </Marker>
        </MapContainer>
        {/* Filtre design mobile-friendly */}
        <div className="absolute inset-0 bg-amber-900/5 pointer-events-none rounded-2xl md:rounded-3xl z-10" />
      </div>
    </motion.div>
  );
}
